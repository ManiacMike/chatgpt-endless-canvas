# ChatGPT Endless Canvas（无限画板）

> ### 🆓 不消耗 Codex 额度
> 用**网页版 ChatGPT** 生图的无限画板——不走付费 API、不占 Codex 额度，
> 用你已有的 ChatGPT 订阅就能一直出图。
...
- 📋 **计划任务批量生图** —— 提交 prompt 列表，任务排队执行、每张之间随机
  间隔 30–120 秒（拟人节奏，避免限流）
- ⚡ **并行生图** —— 最多 3 张同时生成（`BOARD_WORKERS`），每个任务独占一个
  ChatGPT 标签页；任务记录自己的 ChatGPT 会话 uuid，中断后重启会先回到
  该会话把已生成的图取回，而不是重新生成
...
- 🌐 **三语界面** —— 工具栏一键切换 中文 / English / 日本語（按浏览器记住选择）
...
| `POST /api/delete` | `{"name":"<图名>"}` 删除图片及其画布元数据（有任务在用时返回 409） |
| `GET /api/health` | 服务身份：app、版本、项目路径、python、pid、端口、worker 数 |
...
如果 clone 位置不是 `~/Workspace/chatgpt-endless-canvas`，在 shell 配置里
...
| `BOARD_WORKERS` | `3` | 并行生图 worker 数，每个独占一个 ChatGPT 标签页 |
